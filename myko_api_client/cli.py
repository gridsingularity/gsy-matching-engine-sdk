...
import myko_api_client.setups as setups
from myko_api_client.constants import SETUP_FILE_PATH, DEFAULT_DOMAIN_NAME, \
    DEFAULT_WEBSOCKET_DOMAIN, MYKO_CLIENT_SIMULATION_ID
...
def run(base_setup_path, setup_module_name,
    ...
    os.environ["API_CLIENT_DOMAIN_NAME"] = domain_name \
        if domain_name else DEFAULT_DOMAIN_NAME
    os.environ["API_CLIENT_WEBSOCKET_DOMAIN_NAME"] = web_socket \
        if web_socket else DEFAULT_WEBSOCKET_DOMAIN
    os.environ["API_CLIENT_SIMULATION_ID"] = simulation_id \
        if simulation_id else MYKO_CLIENT_SIMULATION_ID
...
def load_client_script(base_setup_path, setup_module_name):
    try:
        if base_setup_path is None:
            importlib.import_module(f"myko_api_client.setups.{setup_module_name}")
