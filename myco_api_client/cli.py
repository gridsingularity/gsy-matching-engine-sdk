...
def run(base_setup_path, setup_module_name,
    ...
    os.environ["MYKO_CLIENT_DOMAIN_NAME"] = domain_name \
        if domain_name else domain_name_from_env()
    os.environ["MYKO_CLIENT_WEBSOCKET_DOMAIN_NAME"] = web_socket \
        if web_socket else websocket_domain_name_from_env()
    os.environ["MYKO_CLIENT_SIMULATION_ID"] = simulation_id \
